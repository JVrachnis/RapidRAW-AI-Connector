#!/usr/bin/env python3
"""mask_points.py - SAM2/SAM3 point/box-prompt masking for the rr-ai-gateway.
...
Usage:
  ...
  mask_points.py IMAGE --box 'x0,y0,x1,y1' --backend sam2 --out mask.png
  mask_points.py IMAGE --box 'x0,y0,x1,y1' --points '[[x,y,1]]' --out mask.png
Point labels: 1 = foreground, 0 = background. Coordinates in image pixels.
At least one of --points/--box is required; both may be given together.
"""
...
def main():
    ...
    ap.add_argument("--points", help="JSON [[x,y,label],...]")
    ap.add_argument("--box", help="'x0,y0,x1,y1' box prompt in image pixels")
    ...
    if not args.points and not args.box:
        ap.error("at least one of --points or --box is required")
